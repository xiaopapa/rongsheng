...
class Mdb_model extends Bmax_Model{
    ...
    public function __construct()
    {
        parent::__construct();
        /*$this->_dbfix = $dbfix;
        $cfg = array('host'=>DB_HOST, 'user'=>DB_USER, 'pwd'=>DB_PWD, 'dbname'=>DB_NAME);
        if($opt && is_array($opt)) $cfg = array_merge($cfg, $opt);
        $k = 'db.'.$cfg['host'].'.'.$cfg['dbname'].'.'.DB_HOST;
        if(o::$temp[$k]){//缓存DB连接
            $this->_conn = o::$temp[$k];
        }else{
            ($this->_conn = @mysql_connect($cfg['host'], $cfg['user'], $cfg['pwd'])) or $this->errorInfo(true, 'mysql_connect: '.$cfg['host'].','.$cfg['user']);
            @mysql_select_db($cfg['dbname'], $this->_conn);
    ...
    public function table($t = ''){
        if($t) $this->_table = $this->_dbfix.str_replace($this->_dbfix,'',$t);
        return $this;
    }
    ...
    public function query($sql, $type = true){
        $this->err = '';
        ($this->result = @mysql_query($sql, $this->_conn)) or ($this->err = $this->errorInfo($type,$sql));
    ...
    public function fetchAll($sql = "", $type = MYSQL_ASSOC){
        if(!$sql) $sql = $this->getSql();
        $result = $this->query($sql);
        while ($array = @mysql_fetch_array($result, $type)) {
            $temp[] = $array;
        }
        return (array)$temp;
    }
    ...
    public function errorInfo($die = true, $msg = ''){
        $msg = 'Date:'.date('Y-m-d H:i:s') .", Errno:". mysql_errno() .', Error:'. mysql_error() . $msg;
        if($die){
            die($msg);
        }else{
