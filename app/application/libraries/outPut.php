...
class outPut{
    ...
    private function arrToStrs($arr,$t,$zip){
        ...
        $addprefix = $arr['addprefix'] ? $arr['addprefix'] : '';
        unset($arr['addprefix']);
        $notcknum = $arr['cknum'] === false;//使用cknum:false表示所在数组不进行数字判断
    ...
    private function phpbegin($desc = ''){
        if(!$desc){
            ...
            foreach($desc as $rs){
                if($rs['class'] === 'o_outfile' && ($rs['function'] === 'php' || $rs['function'] === 'php_define')){
                    $desc = $rs['file'] .':'. $rs['line'];
                    break;
                }
            }
        }
        return "<?php\r\n//cms,". gmdate('Y-m-d H:i:s',time()+3600*8) .",". o::$admin['name'] . $desc . "\r\n";
    }
}